"""
Trixfer base on FGSM
"""
import torch
from adaptation.attacks.Trixfer_Base import Trixfer_Base


class Trixfer_FGSM(Trixfer_Base):
    def __init__(self, eps=8/255, max_value=1., min_value=0., threshold=0., beta=10,
        ...
        super().__init__(eps=eps, max_value=max_value, min_value=min_value,
    ...
    def attack(self, data, fine_label, coarse_label=None, idx=-1):
        """
        Attack with hybrid loss (single step FGSM)
        """
        ...
        # Add perturbation (single step)
        adv_data = self.get_adv_example(ori_data=data, adv_data=adv_data, grad=grad, attack_step=self.attack_step)
        adv_data.detach_()
